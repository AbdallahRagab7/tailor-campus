<template>
  <section class="form-wrapper">
   ...
   -->
      ...
      <div class="form-group">
        <label for="instructor-img">Upload Course img</label>
        <input
          type="file"
          id="instructor-img"
          ref="imageInput"
      ...
      >
        ...
        />
        <div
          v-for="(lesson, lectureIndex) in section.lesson"
          :key="lectureIndex"
        >
          ...
          <input
            type="file"
            @change="handleFileChange(sectionIndex, lectureIndex)"
          />
          <button
            type="button"
            @click="removeLecture(sectionIndex, lectureIndex)"
            class="remove-btn"
          >
            Remove Lecture
          </button>
      ...
      <button type="button" @click="addSection" class="me-5 add-btn">
        + Add Section
      </button>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      //   section.lesson.forEach((lecture, subIndex) => {
    ...
    async submitTextForm() {
      ...
      this.sections.forEach((section, index) => {
        Data.append(`sections[${index}][name]`, section.name);
        section.lesson.forEach((lecture, subIndex) => {
          Data.append(`sections[${index}][lectures][${subIndex}][name]`, lecture.name);
          // Data.append(`sections[${index}][lectures][${subIndex}][video]`, lecture.video);
        });
      });
      Data.append("course_name", this.courseName);
      Data.append("course_price", this.coursePrice);
      Data.append("course_description", this.description);
      Data.append("catgeory", this.category);
      Data.append("level", this.level);
      Data.append("image", this.image);
      // Data.append("sections", this.sections);
